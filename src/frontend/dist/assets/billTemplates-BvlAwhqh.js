...
function companyInfoHeader(info) {
...
function printBill(bill, app, companyInfo) {
  ...
  const lineRows = bill.lineItems.map(
    (li, i) => `<tr>
      <td class="sr">${i + 1}</td>
...
function printReceipt(receipt, bill, app, companyInfo) {
  const header = baroteHeader(companyInfo);
...
function printMonthlyCollectionPDF(receipts, monthYearLabel, companyInfo) {
  ...
  const modeBreakdown = Object.entries(byMode).map(
    ([mode, amt]) => `<tr><td class="lbl">${mode}:</td><td class="val">${fmtCurrency(amt)}</td></tr>`
  ).join("");
  const html = `<div class="page">
    ${header}
    ...
    <div class="footer-note">Generated on ${(/* @__PURE__ */ new Date()).toLocaleDateString("en-IN")}</div>
  </div>`;
  openPrint(html, `Monthly Collection - ${monthYearLabel}`);
}
function printLedgerPDF(entries, appInfo, companyInfo) {
  const header = baroteHeader(companyInfo);
  let running = 0;
  const rows = [...entries].sort((a, b) => Number(a.date) - Number(b.date)).map((e) => {
    running += e.credit - e.debit;
    return { ...e, balance: running };
  });
  const totalDebits = entries.reduce((s, e) => s + e.debit, 0);
  const totalCredits = entries.reduce((s, e) => s + e.credit, 0);
  const entryRows = rows.map(
    (e, i) => `<tr>
        <td>${i + 1}</td>
  ...
  openPrint(html, `Ledger - ${(appInfo == null ? void 0 : appInfo.applicationId) ?? "app"}`);
}
export {
  printReceipt as a,
  printMonthlyCollectionPDF as b,
  printLedgerPDF as c,
  exportToCSV as e,
  printBill as p
};
